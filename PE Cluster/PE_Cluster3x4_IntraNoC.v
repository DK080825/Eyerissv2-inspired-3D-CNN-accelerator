// ================================================================================================ //
// 3x4 PE-cluster intra-NoC production primitives.
//
// Layout:
//   PE index = row * PE_COLS + col, with PE_ROWS=3 and PE_COLS=4.
//
// Production modules:
//   PE3x4_WEIGHT_RowFabric        - one Weight lane per PE row.
//   PE3x4_IACT_Atomic_Mask_Fabric - fixed-order IACT slot-to-PE mask fabric.
...
module PE3x4_IACT_Atomic_Mask_Fabric #(
    ...
    always @(*) begin
        ...
        for (lane_pos = 0; lane_pos < SRC_COUNT; lane_pos = lane_pos + 1) begin
            if (src_valid_in[lane_pos]) begin
                eff_mask_w = src_dst_mask_in[(lane_pos*PE_COUNT) +: PE_COUNT] & active_pe_mask_in;
                if (eff_mask_w == {PE_COUNT{1'b0}}) begin
                    src_ready_w[lane_pos] = 1'b1;
                end else begin
                    all_ready_w = 1'b1;
                    ...
                    src_ready_w[lane_pos] = lane_can_fire_w;

                    if (lane_can_fire_w) begin
                        for (pe_idx = 0; pe_idx < PE_COUNT; pe_idx = pe_idx + 1) begin
                            if (eff_mask_w[pe_idx]) begin
                                pe_valid_w[pe_idx] = 1'b1;
                                pe_data_w[(pe_idx*DATA_W) +: DATA_W] = src_data_in[(lane_pos*DATA_W) +: DATA_W];
